//"version": "1.0.2" "https://github.com/vtHawk/three-csm.git"
...
class Frustum {
  ...
  split(breaks) {
    ...
    for (let i = 0; i < breaks.length; i++) {
      ...
      if (i === 0) {
        ...
        for (let j = 0; j < 4; j++) {
          cascade.vertices.near.push(
            new FrustumVertex().fromLerp(
  ...
  toSpace(cameraMatrix) {
    ...
    for (var i = 0; i < 4; i++) {
      point.set(
        this.vertices.near[i].x,
        this.vertices.near[i].y,
        this.vertices.near[i].z
      )
      point.applyMatrix4(cameraMatrix)
      result.vertices.near.push(new FrustumVertex(point.x, point.y, point.z))

      point.set(
        this.vertices.far[i].x,
        this.vertices.far[i].y,
        this.vertices.far[i].z
      )
      point.applyMatrix4(cameraMatrix)
...
class CSM {
  ...
  getBreaks() {
    ...
    switch (this.mode) {
      ...
      case "custom":
        ...
        this.breaks = this.customSplitsCallback(
          this.cascades,
          this.near,
          this.far
        )
  ...
  update(cameraMatrix) {
    for (let i = 0; i < this.frustums.length; i++) {
      ...
      const lightSpaceFrustum = worldSpaceFrustum.toSpace(
